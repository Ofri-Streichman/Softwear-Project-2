...
/*copies 2d array to other 2d array*/
...
/* the main function that calculates the centroids */
...
static PyObject* fit_capi(PyObject *self, PyObject *args)
{
    ...
    if(!PyArg_ParseTuple(args, "iiiiOO", &k,&n,&d,&max,&PyList1,&PyList2)) {
        return NULL;
    }
    ...
    assert(vectors != NULL);
    for (p = 0; p < n; p++) {
        vectors[p] = (double *) malloc(d * sizeof(double));
        assert(vectors[p] != NULL);
    }
    centroids = (double **) malloc(k * sizeof(double *));
    assert(centroids != NULL);
    for (p = 0; p < k; p++) {
        centroids[p] = (double *) malloc(d * sizeof(double));
        assert(centroids[p] != NULL);
    }


    /* checking if the PyObjects are lists */
    if (!PyList_Check(PyList1)||!PyList_Check(PyList2)||!PyList_Check(result_lst)) {
        return NULL;
    }

    /* making vectors array and centroids array with the PyObjects */
    for (i = 0; i < k; i++) {
        item = PyList_GetItem(PyList2, i);
        PyObject *item_inside;
        for (j = 0; j < d; j++) {
            item_inside = PyList_GetItem(item, j);
            centroids[i][j] = PyFloat_AsDouble(item_inside);
        }
    }

    for (i = 0; i < n; i++) {
        item2 = PyList_GetItem(PyList1, i);
        PyObject *item_inside;
        for (j = 0; j < d; j++) {
            item_inside = PyList_GetItem(item2, j);
            vectors[i][j] = PyFloat_AsDouble(item_inside);
        }
    }
    ...
    /* making PyList of the final centroids */
    for (i = 0; i < k; i++) {
